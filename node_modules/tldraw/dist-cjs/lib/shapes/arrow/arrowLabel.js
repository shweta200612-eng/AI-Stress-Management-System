...
function getArrowDisplayValues(editor, shape) {
  return (0, import_getDisplayValues.getDisplayValues)(editor.getShapeUtil("arrow"), shape);
}
...
const labelSizeCache = (0, import_editor.createComputedCache)(
  ...
  (editor, shape) => {
    ...
    return new import_editor.Vec(width, height).addScalar(dv.labelPadding * 2 * shape.props.scale);
  },
  {
    areRecordsEqual: (a, b) => {
...
function getLabelToArrowPadding(shape, theme) {
  const strokeWidth = theme.strokeWidth * import_default_shape_constants.STROKE_SIZES[shape.props.size];
  const smallStrokeWidth = theme.strokeWidth * import_default_shape_constants.STROKE_SIZES.s;
  const xlStrokeWidth = theme.strokeWidth * import_default_shape_constants.STROKE_SIZES.xl;
  const labelToArrowPadding = (import_default_shape_constants.LABEL_TO_ARROW_PADDING + (strokeWidth - smallStrokeWidth) * 2 + (strokeWidth >= xlStrokeWidth ? 20 : 0)) * shape.props.scale;
