...
__export(shared_exports, {
  ...
  clampArrowTerminalToMask: () => clampArrowTerminalToMask,
...
function getArrowTerminalInArrowSpace(editor, arrowPageTransform, binding, forceImprecise) {
  ...
  if (!boundShape) {
    ...
    const shapePoint = import_editor.Vec.Add(point, import_editor.Vec.MulV(normalizedAnchor, size));
...
function clampArrowTerminalToMask(editor, point, terminalHandle, arrowPageTransform, targetShapeInfo) {
  ...
  if (!mask) return;
  const pagePoint = import_editor.Mat.applyToPoint(arrowPageTransform, point);
  if ((0, import_editor.pointInPolygon)(pagePoint, mask)) return;
  const pageAnchor = import_editor.Mat.applyToPoint(arrowPageTransform, terminalHandle);
  const direction = import_editor.Vec.Sub(pageAnchor, pagePoint).uni();
  const extendedAnchor = import_editor.Vec.Add(pageAnchor, import_editor.Vec.Mul(direction, 1));
  const intersections = (0, import_editor.intersectLineSegmentPolygon)(extendedAnchor, pagePoint, mask);
  if (!intersections || intersections.length === 0) return;
  let closest = intersections[0];
  let closestDist = import_editor.Vec.Dist2(closest, pagePoint);
  for (let i = 1; i < intersections.length; i++) {
    const dist = import_editor.Vec.Dist2(intersections[i], pagePoint);
    if (dist < closestDist) {
      closest = intersections[i];
      closestDist = dist;
    }
  }
  const arrowPoint = import_editor.Mat.applyToPoint(import_editor.Mat.Inverse(arrowPageTransform), closest);
