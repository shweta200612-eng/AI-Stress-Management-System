...
function getCurvedArrowInfo(editor, shape, bindings, arrowStrokeWidth) {
  ...
  if (Math.abs(bend) > Math.abs(shape.props.bend * (import_shared.WAY_TOO_BIG_ARROW_BEND_FACTOR * shape.props.scale))) {
    return (0, import_straight_arrow.getStraightArrowInfo)(editor, shape, bindings, arrowStrokeWidth);
  }
  ...
  const arrowSW = arrowStrokeWidth ?? theme.strokeWidth * import_default_shape_constants.STROKE_SIZES[shape.props.size];
  ...
  if (startShapeInfo && !startShapeInfo.isExact) {
    ...
    if (point) {
      ...
      if (arrowheadStart !== "none") {
        const strokeOffset = arrowSW / 2 + ("size" in startShapeInfo.shape.props ? theme.strokeWidth * import_default_shape_constants.STROKE_SIZES[startShapeInfo.shape.props.size] / 2 : 0);
  ...
  if (endShapeInfo && !endShapeInfo.isExact) {
    ...
    if (point) {
      ...
      if (arrowheadEnd !== "none") {
        const strokeOffset = arrowSW / 2 + ("size" in endShapeInfo.shape.props ? theme.strokeWidth * import_default_shape_constants.STROKE_SIZES[endShapeInfo.shape.props.size] / 2 : 0);
        offsetB = (import_shared.BOUND_ARROW_OFFSET + strokeOffset) * shape.props.scale;
        minLength += strokeOffset * shape.props.scale;
      }
    }
  }
  (0, import_shared.clampArrowTerminalToMask)(editor, tempA, a, arrowPageTransform, startShapeInfo);
  (0, import_shared.clampArrowTerminalToMask)(editor, tempB, b, arrowPageTransform, endShapeInfo);
