...
function getElbowArrowInfo(editor, arrow, bindings, arrowStrokeWidth) {
...
function getElbowArrowTerminalInfo(editor, arrow, binding, point, arrowStrokeWidth) {
  const theme = editor.getCurrentTheme();
  const arrowSW = arrowStrokeWidth ?? theme.strokeWidth * import_default_shape_constants.STROKE_SIZES[arrow.props.size];
  ...
  if (binding) {
    ...
    if (geometry && target) {
      ...
      if (arrow.props[arrowheadProp] !== "none") {
        ...
        const targetStrokeSize = "size" in target.props ? theme.strokeWidth * (import_default_shape_constants.STROKE_SIZES[target.props.size] ?? 0) * targetScale / 2 : 0;
