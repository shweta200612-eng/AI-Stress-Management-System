...
class Drawing extends import_editor.StateNode {
  ...
  onPointerMove() {
    ...
    if (this.isPenOrStylus) {
      ...
      if (import_editor.Vec.Dist(currentPagePoint, this.lastRecordedPoint) >= 1 / this.zoomOnEnter) {
  ...
  getIsClosed(segments, size, scale, strokeWidth) {
    ...
    const firstPoint = import_editor.b64Vecs.decodeFirstPoint(segments[0].path, segments[0].dim);
    const lastSegment = segments[segments.length - 1];
    const lastPoint = import_editor.b64Vecs.decodeLastPoint(lastSegment.path, lastSegment.dim);
    const isDynamicResizingEnabled = this.editor.user.getIsDynamicResizeMode();
    const threshold = isDynamicResizingEnabled ? (strokeWidth + 2) * scale : (
      // 6 is a base floor, 2 is stroke influence, 0.8 tempers width growth
      6 + 2 * Math.sqrt(strokeWidth * 0.8) + // 100 is low-zoom boost, 0.18 is the zoom knee, 3 controls falloff steepness
      100 / (1 + Math.pow(this.zoomOnEnter / 0.18, 3))
    );
    return firstPoint !== null && lastPoint !== null && firstPoint !== lastPoint && this.currentLineLength > strokeWidth * 4 * scale && import_editor.Vec.DistMin(firstPoint, lastPoint, threshold);
  }
  /**
   ...
   */
  makeSegment(type, points) {
    const path = import_editor.b64Vecs.encodePoints(points, this.segmentDim);
    return this.segmentDim === import_editor.DIM_2D ? { type, path, dim: import_editor.DIM_2D } : { type, path };
  }
  startShape() {
    ...
    this.segmentDim = this.isPenOrStylus ? void 0 : import_editor.DIM_2D;
    ...
    if (this.initialShape) {
      ...
      if (shape2 && this.segmentMode === "straight") {
        ...
        const prevPoint = import_editor.b64Vecs.decodeLastPoint(prevSegment.path, prevSegment.dim);
        ...
        const dvStrokeWidth = (0, import_getDisplayValues.getDisplayValues)(this.util, shape2).strokeWidth;
    ...
    switch (this.segmentMode) {
      case "starting_straight": {
        ...
        if (hasMovedFarEnough) {
          ...
          if (!prevSegment) throw Error("Expected a previous segment!");
          const prevLastPoint = import_editor.b64Vecs.decodeLastPoint(prevSegment.path, prevSegment.dim);
      ...
      case "starting_free": {
        ...
        if (hasMovedFarEnough) {
          ...
          const prevPoint = import_editor.b64Vecs.decodeLastPoint(
          ...
          const dvStrokeWidth = (0, import_getDisplayValues.getDisplayValues)(this.util, shape).strokeWidth;
          if (this.currentLineLength < dvStrokeWidth * 4) {
      ...
      case "straight": {
        ...
        if (shouldSnap) {
          if (newSegments.length > 2) {
            ...
            for (let i = 0, n = segments.length - 2; i < n; i++) {
              ...
              const first = import_editor.b64Vecs.decodeFirstPoint(segment.path, segment.dim);
              const lastPoint = import_editor.b64Vecs.decodeLastPoint(segment.path, segment.dim);
        ...
        if (didSnap && snapSegment) {
          const transform = this.editor.getShapePageTransform(shape);
          const first = import_editor.b64Vecs.decodeFirstPoint(snapSegment.path, snapSegment.dim);
          const lastPoint = import_editor.b64Vecs.decodeLastPoint(snapSegment.path, snapSegment.dim);
        ...
        this.currentLineLength += newSegments.length && import_editor.b64Vecs.decodeFirstPoint(newSegment.path, newSegment.dim) ? import_editor.Vec.Dist(
          import_editor.b64Vecs.decodeFirstPoint(newSegment.path, newSegment.dim),
        ...
        newSegments[newSegments.length - 1] = {
          ...
          path: import_editor.b64Vecs.encodePoints(
            [import_editor.b64Vecs.decodeFirstPoint(newSegment.path, newSegment.dim), import_editor.Vec.From(newPoint2)],
            newSegment.dim
          )
        };
        const shapePartial = {
      ...
      case "free": {
        ...
        newSegments[newSegments.length - 1] = {
          ...newSegment,
          path: import_editor.b64Vecs.encodePoints(cachedPoints, newSegment.dim)
        };
        const dvStrokeWidth = (0, import_getDisplayValues.getDisplayValues)(this.util, shape).strokeWidth;
        if (this.currentLineLength < dvStrokeWidth * 4) {
  ...
  getLineLength(segments) {
    ...
    for (let j = 0; j < segments.length; j++) {
      const points = import_editor.b64Vecs.decodePoints(segments[j].path, segments[j].dim);
