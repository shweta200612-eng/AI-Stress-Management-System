...
__export(getPath_exports, {
  ...
  getPointsFromDrawSegment: () => getPointsFromDrawSegment,
  getPointsFromDrawSegments: () => getPointsFromDrawSegments
});
...
function getPointsFromDrawSegment(segment, scaleX, scaleY, points = []) {
  const _points = import_editor.b64Vecs.decodePoints(segment.path, segment.dim);
  ...
  if (segment.type === "free" || _points.length < 2) {
    points.push(..._points.map(import_editor.Vec.From));
  } else {
    const A = import_editor.Vec.From(_points[0]);
    const D = import_editor.Vec.From(_points[1]);
    const dist = import_editor.Vec.Dist(D, A);
    if (dist === 0) {
      points.push(A);
    } else {
      const uni = import_editor.Vec.Tan(D, A);
      const nudgeDist = Math.min(1, Math.floor(dist / 4));
      const B = import_editor.Vec.Add(A, import_editor.Vec.Mul(uni, nudgeDist));
      const C = import_editor.Vec.Add(D, import_editor.Vec.Mul(uni, -nudgeDist));
      const interpolatedPointsCount = Math.max(4, Math.floor(dist / 16));
      points.push(A, ...import_editor.Vec.PointsBetween(B, C, interpolatedPointsCount, import_editor.EASINGS.easeInOutCubic), D);
    }
  }
  return points;
}
