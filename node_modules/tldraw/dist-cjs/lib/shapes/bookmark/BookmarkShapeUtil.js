...
class BookmarkShapeUtil extends import_editor.BaseBoxShapeUtil {
  ...
  getGeometry(shape) {
    return new import_editor.Rectangle2d({
      width: shape.props.w,
      height: (0, import_bookmarks.getBookmarkShapeHeight)(this.editor, shape),
      isFilled: true
    });
  }
  component(shape) {
    const { assetId, url } = shape.props;
    const h = (0, import_bookmarks.getBookmarkShapeHeight)(this.editor, shape);
    const rotation = this.editor.getShapePageTransform(shape).rotation();
    return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(BookmarkShapeComponent, { assetId, url, h, rotation });
  }
  getIndicatorPath(shape) {
    const path = new Path2D();
    path.rect(0, 0, shape.props.w, (0, import_bookmarks.getBookmarkShapeHeight)(this.editor, shape));
    return path;
  }
  onBeforeCreate(next) {
...
function BookmarkShapeComponent({
  ...
  const resolvedAssetId = (0, import_bookmarks.getResolvedBookmarkAssetId)(editor, assetId, url);
  const asset = resolvedAssetId ? editor.getAsset(resolvedAssetId) : null;
