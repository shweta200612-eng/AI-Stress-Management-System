...
const BASE_NOTE_POSITIONS = (editor, noteWidth, noteHeight) => [
  [
    ["a1"],
    new import_editor.Vec(noteWidth * 0.5, noteHeight * -0.5 - editor.options.adjacentShapeMargin)
  ],
  // t
  [
    ["a2"],
    new import_editor.Vec(noteWidth * 1.5 + editor.options.adjacentShapeMargin, noteHeight * 0.5)
  ],
  // r
  [
    ["a3"],
    new import_editor.Vec(noteWidth * 0.5, noteHeight * 1.5 + editor.options.adjacentShapeMargin)
  ],
  // b
  [
    ["a4"],
    new import_editor.Vec(noteWidth * -0.5 - editor.options.adjacentShapeMargin, noteHeight * 0.5)
  ]
  // l
];
function getBaseAdjacentNotePositions(editor, scale, noteWidth, noteHeight) {
  ...
  return [
    [["a1"], new import_editor.Vec(w * 0.5, h * -0.5 - m)],
    // t
    [["a2"], new import_editor.Vec(w * 1.5 + m, h * 0.5)],
    // r
    [["a3"], new import_editor.Vec(w * 0.5, h * 1.5 + m)],
    // b
    [["a4"], new import_editor.Vec(w * -0.5 - m, h * 0.5)]
    // l
  ];
}
  ...
  if (!nextNote || forceNew) {
    ...
    const topLeft = editor.getPointInParentSpace(
      ...
      import_editor.Vec.Sub(
        center,
        import_editor.Vec.Rot(new import_editor.Vec(noteWidth / 2, noteHeight / 2).mul(createdShape.props.scale), pageRotation)
      )
    );
    editor.updateShape({
      id,
