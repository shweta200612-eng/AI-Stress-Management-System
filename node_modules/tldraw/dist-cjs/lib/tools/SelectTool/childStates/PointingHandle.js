...
var import_getDisplayValues = require("../../../shapes/shared/getDisplayValues");
...
class PointingHandle extends import_editor.StateNode {
  ...
  startDraggingHandle() {
    ...
    if (editor.isShapeOfType(shape, "note")) {
      ...
      if (nextNote) {
        const centeredOnPointer = editor.getPointInParentSpace(nextNote, editor.inputs.getOriginPagePoint()).sub(
          import_editor.Vec.Rot(
            new import_editor.Vec(dv.noteWidth / 2, dv.noteHeight / 2).mul(shape.props.scale),
            nextNote.rotation
          )
        );
...
function getNoteForAdjacentPosition(editor, shape, handle, forceNew) {
  ...
  const positions = (0, import_noteHelpers.getNoteAdjacentPositions)(editor, {
    pagePoint,
    pageRotation,
  ...
  if (position) {
    return (0, import_noteHelpers.getNoteShapeForAdjacentPosition)(editor, {
      shape,
      center: position,
      pageRotation,
      noteWidth: dv.noteWidth,
