...
function iOS() {
  ...
  return ["iPad Simulator", "iPhone Simulator", "iPod Simulator", "iPad", "iPhone", "iPod"].includes(
    ...
    (0, import_dom.getGlobalWindow)().navigator.platform
  ) || // iPad on iOS 13 detection
  import_environment.tlenv.isDarwin && "ontouchend" in (0, import_dom.getGlobalDocument)();
}
...
if (typeof window !== "undefined") {
  (0, import_dom.getGlobalWindow)().addEventListener("beforeunload", () => {
    (0, import_utils.setInSessionStorage)(tabIdKey, TAB_ID);
  });
}
const Versions = {
...
function loadSessionStateSnapshotIntoStore(store, snapshot, opts) {
  ...
  if (!res) return;
  const existingInstance = store.get(import_tlschema.TLINSTANCE_ID);
  const preserved = (0, import_tlschema.pluckPreservingValues)(existingInstance);
